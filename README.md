...
```
Note that this sample `input.json` file specifies as working directory for the judge a directory named `working_dir` at the path determined by `$JUDGE_HOME`.

8. Run the test case by running the ``judge_check`` script.
```
judge_check
```
The output of that script is the raw JSON-formated output file generated by GTest.
